...
import com.bulletnoid.android.widget.StaggeredGridView.ScrollerCompat;
 ...
 * </p>
 */
public class StaggeredGridViewByJaumo extends ViewGroup
{
	...
	public StaggeredGridViewByJaumo(Context context)
	{
		this(context, null);
	}

	public StaggeredGridViewByJaumo(Context context, AttributeSet attrs)
	{
	...
	public void setOnChangedScrollDirectionListener(
			OnChangedScrollDirectionListener changedScrollDirectionListener)
	{
	...
	private boolean trackMotionScroll(int deltaY, boolean allowOverScroll)
	{
		...
		if (!contentFits)
		{
			...
			int scrollDirection = deltaY > 0 ? ScrollDirection.UP
					: ScrollDirection.DOWN;
			if (Math.abs(deltaY) > 10
					&& mChangedScrollDirectionListener != null
					&& scrollDirection != mScrollDirection)
			{
				long time = new Date().getTime();
				if (mScrollDirectionChangeDate < time - 1000)
				{
					mChangedScrollDirectionListener
							.onScrollDirectionChanged(scrollDirection);
	...
	private int getColWidth(int position)
	{
		...
		return (getWidth() - getPaddingLeft() - getPaddingRight() - itemMargin
				* 2 - itemMargin * (mColCount - 1))
				/ mColCount;
	}
	...
	final void layoutChildren(boolean queryAdapter)
	{
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			final int childLeft = paddingLeft + itemMargin + col
					* (colWidth + itemMargin);
	...
	final int fillUp(int fromPosition, int overhang)
	{
		...
		while (nextCol >= 0 && mItemTops[nextCol] > fillTo && position >= 0)
		{
			...
			final int childLeft = paddingLeft + itemMargin + nextCol
					* (colWidth + itemMargin);
			final int childRight = childLeft + child.getMeasuredWidth();
	...
	final int fillDownTo(int position, int fillTo)
	{
				...
				&& position < mItemCount)
		{
			...
			final int span = Math.min(mColCount, lp.span);
			final int widthSize = colWidth * span + getItemMargin(position)
					* (span - 1);
			final int widthSpec = MeasureSpec.makeMeasureSpec(widthSize,
					MeasureSpec.EXACTLY);
			...
			{
				childLeft = paddingLeft + itemMargin + nextCol
						* (colWidth + itemMargin);
			}
			final int childRight = childLeft + child.getMeasuredWidth();
			child.layout(childLeft, childTop, childRight, childBottom);
			...
			if (this.mColWidth > 0)
				for (int i = 0; i < mColCount; i++)
				{
					if (getChildAt(i) != null)
					{
						...
						while (left > col * (this.mColWidth + itemMargin * 2)
								+ getPaddingLeft())
						{
							col++;
						}
	...
	private class AdapterDataSetObserver extends DataSetObserver
	{
		...
		public void onChanged()
		{
			...
			} else
			{
				...
				// changed
				//causes the scroller to go to top when notifydatachanged() is invoked
	...
	public interface OnItemClickListener
	{
		 ...
		 */
		void onItemClick(StaggeredGridViewByJaumo staggeredGridViewNew, View view, int position,
				long id);
	}
	...
	public interface OnItemLongClickListener
	{
		 ...
		 */
		boolean onItemLongClick(StaggeredGridViewByJaumo staggeredGridViewNew, View view,
