...
public class StaggeredGridView extends ViewGroup
{
	...
	{
		mScroller.startScroll(0, 1, 0, top);
		invalidate();
		requestLayout();
	}

	/*
	 * public void fullScrollToTop() { mScroller.startScroll(0,
	 * getChildCount()*mFirstPosition, 0,getChildCount()*mFirstPosition); }
	 */

	public int getScollval()
	{
	...
	public void setTouchMode(int newState)
	{
		...
		case TOUCH_MODE_IDLE:
			...
			break;
		case TOUCH_MODE_TAP:
			reportScrollStateChange(OnScrollListener.SCROLL_STATE_TAP);
			break;
		case TOUCH_MODE_REST:
			reportScrollStateChange(OnScrollListener.SCROLL_STATE_REST);
			break;
		}
	}
	 ...
	 * 
	 * mVelocityTracker.clear(); mScroller.abortAnimation(); mLastTouchY =
	 * ev.getY(); mLastTouchX = ev.getX(); motionPosition =
	 * pointToPosition((int) mLastTouchX, (int) mLastTouchY); mActivePointerId =
	 * MotionEventCompat.getPointerId(ev, 0); mTouchRemainderY = 0;
	 * 
	 * if (mTouchMode != TOUCH_MODE_FLINGING && !mDataChanged && motionPosition
	 * >= 0 && getAdapter().isEnabled(motionPosition)) { mTouchMode =
	 * TOUCH_MODE_DOWN;
	 * 
	 * mBeginClick = true;
	 * 
	 * if (mPendingCheckForTap == null) { mPendingCheckForTap = new
	 * CheckForTap(); }
	 * 
	 * postDelayed(mPendingCheckForTap, ViewConfiguration.getTapTimeout()); }
	 * 
	 * mMotionPosition = motionPosition; invalidate(); break;
	 * 
	 * case MotionEvent.ACTION_MOVE:
	 * 
	 * final int index = MotionEventCompat.findPointerIndex(ev,
	 * mActivePointerId); if (index < 0) { Log.e(TAG,
	 * "onInterceptTouchEvent could not find pointer with id " +
	 * mActivePointerId + " - did StaggeredGridView receive an inconsistent " +
	 * "event stream?"); return false; } final float y =
	 * MotionEventCompat.getY(ev, index); final float dy = y - mLastTouchY +
	 * mTouchRemainderY; final int deltaY = (int) dy; mTouchRemainderY = dy -
	 * deltaY;
	 * 
	 * if (Math.abs(dy) > mTouchSlop) { mTouchMode = TOUCH_MODE_DRAGGING;
	 * setTouchMode(mTouchMode); }
	 * 
	 * if (mTouchMode == TOUCH_MODE_DRAGGING) { mLastTouchY = y;
	 * 
	 * if (!trackMotionScroll(deltaY, true)) { // Break fling velocity if we
	 * impacted an edge. mVelocityTracker.clear(); } }
	 * 
	 * updateSelectorState(); break;
	 * 
	 * case MotionEvent.ACTION_CANCEL: mTouchMode = TOUCH_MODE_IDLE;
	 * setTouchMode(mTouchMode); updateSelectorState(); setPressed(false); View
	 * motionView = this.getChildAt(mMotionPosition - mFirstPosition); if
	 * (motionView != null) { motionView.setPressed(false); } final Handler
	 * handler = getHandler(); if (handler != null) {
	 * handler.removeCallbacks(mPendingCheckForLongPress); }
	 * 
	 * if (mTopEdge != null) { mTopEdge.onRelease(); mBottomEdge.onRelease(); }
	 * 
	 ...
	 * mVelocityTracker.computeCurrentVelocity(1000, mMaximumVelocity); final
	 * float velocity = VelocityTrackerCompat.getYVelocity(mVelocityTracker,
	 * mActivePointerId); final int prevTouchMode = mTouchMode;
	 * 
	 * if (Math.abs(velocity) > mFlingVelocity) { // TODO mTouchMode =
	 * TOUCH_MODE_FLINGING; setTouchMode(mTouchMode); mScroller.fling(0, 0, 0,
	 * (int) velocity, 0, 0, Integer.MIN_VALUE, Integer.MAX_VALUE); mLastTouchY
	 * = 0; invalidate(); } else { mTouchMode = TOUCH_MODE_IDLE;
	 * setTouchMode(mTouchMode); }
	 * 
	 * if (!mDataChanged && mAdapter.isEnabled(motionPosition)) { // TODO :
	 * handle mTouchMode = TOUCH_MODE_TAP; } else { mTouchMode =
	 * TOUCH_MODE_REST; }
	 * 
	 * switch (prevTouchMode) { case TOUCH_MODE_DOWN: case TOUCH_MODE_TAP: case
	 * TOUCH_MODE_DONE_WAITING: final View child = getChildAt(motionPosition -
	 * mFirstPosition); final float x = ev.getX(); final boolean inList = x >
	 * getPaddingLeft() && x < getWidth() - getPaddingRight(); if (child != null
	 * && !child.hasFocusable() && inList) { if (mTouchMode != TOUCH_MODE_DOWN)
	 * { child.setPressed(false); }
	 * 
	 * if (mPerformClick == null) { invalidate(); mPerformClick = new
	 * PerformClick(); }
	 * 
	 ...
	 * final Handler handlerTouch = getHandler(); if (handlerTouch != null) {
	 * handlerTouch.removeCallbacks(mTouchMode == TOUCH_MODE_DOWN ?
	 * mPendingCheckForTap : mPendingCheckForLongPress); }
	 * 
	 * if (!mDataChanged && mAdapter.isEnabled(motionPosition)) { mTouchMode =
	 * TOUCH_MODE_TAP;
	 * 
	 * layoutChildren(mDataChanged); child.setPressed(true);
	 * positionSelector(mMotionPosition, child); setPressed(true); if (mSelector
	 * != null) { Drawable d = mSelector.getCurrent(); if (d != null && d
	 * instanceof TransitionDrawable) { ((TransitionDrawable)
	 * d).resetTransition(); } } if (mTouchModeReset != null) {
	 * removeCallbacks(mTouchModeReset); } mTouchModeReset = new Runnable() {
	 * 
	 * @Override public void run() { mTouchMode = TOUCH_MODE_REST;
	 * child.setPressed(false); setPressed(false); if (!mDataChanged) {
	 * performClick.run(); } } }; postDelayed(mTouchModeReset,
	 * ViewConfiguration.getPressedStateDuration());
	 * 
	 * } else { mTouchMode = TOUCH_MODE_REST; } return true; } else if
	 ...
	 * mBeginClick = false;
	 * 
	 * updateSelectorState(); } break; } return true; }
	 */
	@Override
	...
	private boolean trackMotionScroll(int deltaY, boolean allowOverScroll)
	{
		...
		if (!contentFits)
		{
			...
			if (mLoadListener != null
					&& !loadlock
					&& deltaY < 0
					&& (mFirstPosition + getChildCount()) > (mAdapter
							.getCount() * 0.75))
			{
				mLoadListener.onLoadmore();
				loadlock = true;
			...
			boolean canJump = deltaY > 0
					&& (mFirstPosition / Math.max(1, mColCount)) > 2;
					...
					else
					{
						if (newViewIdx < i)
						{
							throw new RuntimeException(
									"Cannot recycle child in layout with lower index");
						}
	...
	public interface OnItemClickListener
	{
		 ...
		 */
		void onItemClick(StaggeredGridView staggeredGridViewFlux, View view,
	...
	public interface OnItemLongClickListener
	{
		 ...
		 */
		boolean onItemLongClick(StaggeredGridView staggeredGridViewFlux,
	...
	public interface OnScrollListener
	{
		...
		public static int SCROLL_STATE_TAP = 3;
		
		public static int SCROLL_STATE_REST = 4;
